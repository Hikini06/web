...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['item_id']) && isset($_FILES['image'])) {
        ...
        $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/image/items-img/';
        ...
        try {
            ...
            $stmt->execute([':img' => $new_filename, ':id' => $item_id]);

            echo json_encode(['success' => true, 'image_path' => '/image/items-img/' . $new_filename . '?' . time(), 'message' => 'Tải lên ảnh thành công.']);
            exit();
